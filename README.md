...
<head>
    ...
    <style>
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .top-yt-link { position: absolute; top: 20px; right: 20px; text-decoration: none; z-index: 9999; }
        .yt-img { width: 60px; height: 60px; border-radius: 50%; border: 3px solid #ff004d; object-fit: cover; background: #222; }
        .card { background: #111; border: 1px solid #222; border-radius: 25px; width: 95%; max-width: 400px; height: 85vh; display: flex; flex-direction: column; padding: 15px; box-shadow: 0 20px 60px rgba(0,0,0,1); }
        ...
        .msg { padding: 10px 14px; border-radius: 18px; font-size: 14px; line-height: 1.4; word-wrap: break-word; }
...
<script type="module">
    ...
    document.getElementById('avatarInput').addEventListener('change', function(e) {
        ...
        reader.onload = (ev) => {
            ...
            img.onload = () => {
                ...
                userAvatar = canvas.toDataURL('image/jpeg', 0.5); // Сжимаем до минимума
    ...
    const sendMsg = () => {
        ...
        // Если аватарка сломалась (черный квадрат), ставим дефолт
        const finalAv = (userAvatar.length < 500) ? 'https://pravatar.cc' : userAvatar;

        push(messagesRef, { 
            userId, avatar: finalAv, text, timestamp: serverTimestamp() 
        }).then(() => {
            input.value = "";
        }).catch(err => alert("Ошибка базы!"));
    };
