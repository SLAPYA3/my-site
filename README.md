...
<head>
    ...
    <style>
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .yt-img { width: 60px; height: 60px; border-radius: 50%; border: 3px solid #ff004d; object-fit: cover; background: #222; }
        ...
        .avatar, .current-user-av { width: 35px; height: 35px; border-radius: 50%; border: 1.5px solid #ff004d; object-fit: cover; background: #333; flex-shrink: 0; }
        ...
        /* НОВАЯ ИКОНКА САМОЛЕТА ДЛЯ ВЫБОРА ФОТО */
        .file-label { cursor: pointer; font-size: 22px; transition: 0.2s; opacity: 0.8; display: flex; align-items: center; }
        .file-label:hover { opacity: 1; color: #ff004d; transform: rotate(-15deg); }
        #avatarInput { display: none; }
        
        .send-btn { background: #ff004d; color: #fff; border: none; border-radius: 50%; width: 35px; height: 35px; cursor: pointer; font-weight: bold; display: flex; align-items: center; justify-content: center; }
    </style>
</head>
<body>
    ...
    <div class="card">
        <div class="header">
            ...
            <!-- ЗАМЕНИЛИ ФОТОАППАРАТ НА САМОЛЕТИК -->
            <label class="file-label" title="Выбрать аву">✈️<input type="file" id="avatarInput" accept="image/*"></label>
        </div>
...
<script type="module">
    ...
    let userAvatar = localStorage.getItem('userAvatar') || 'https://pravatar.cc' + userId;
    const myImgDisplay = document.getElementById('myCurrentAv');
    myImgDisplay.src = userAvatar;
    ...
    onChildAdded(messagesRef, (data) => {
        ...
        const displayAv = msg.avatar || 'https://pravatar.cc';
        wrapper.innerHTML = `<img src="${displayAv}" class="avatar"><div class="msg">${msg.text}</div>`;
